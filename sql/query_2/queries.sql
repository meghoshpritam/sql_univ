...
FROM SPJ, S
WHERE SPJ.S = S.S AND QTY >= 300 AND QTY <= 700;
...
FROM S, P, J, SPJ
WHERE S.S = SPJ.S AND P.P = SPJ.P AND J.J = SPJ.J AND 
...
FROM S, P, J, SPJ
WHERE S.S = SPJ.S AND P.P = SPJ.P AND J.J = SPJ.J AND 
S.CITY != P.CITY AND S.CITY != J.CITY AND P.CITY != J.CITY;
...
FROM S, P, J, SPJ
WHERE S.S = SPJ.S AND P.P = SPJ.P AND J.J = SPJ.J AND 
S.CITY = 'London' AND J.CITY = 'London';
...
FROM S, P, J, SPJ
WHERE S.CITY = J.CITY AND S.S =SPJ.S AND P.P = SPJ.P AND J.J = SPJ.J;
...
FROM SPJ AS P1, SPJ AS P2
WHERE P1.S = P2.S AND P1.P != P2.P;


-- 15 Get the total number of projects supplied by supplier S1
SELECT COUNT(SPJ.P) FROM SPJ
WHERE SPJ.S = 'S1';
